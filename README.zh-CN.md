...
本地反向代理 / tunnel 绑定、真实公网回路探测、GPT Builder 操作记录属于私有 ops 仓库，不应放在公开仓库里。
...
- [x] 落地 ChatGPT 直驱开发模式：writeFile / editFile / listDirectory / searchCode / runShell / getGitDiff / getGitStatus / gitCommit，并对 diff / commit / Codex artifacts 做 public-safe 路径过滤
- [x] GPT Actions 边界探测：超时 ~30s、上下文 ≥ 53KB、串行/并行无限流
- [x] 提供 `templates/` 首任务模板库
- [x] 提供 `examples/` 新手案例
...
- `listDirectory` — 列目录
- `searchCode` — 代码搜索
- `runShell` — 白名单命令执行（npm / tsc / vitest / cargo...），25s 上限，适合短验证
- `getGitDiff` / `getGitStatus` / `gitCommit` — Git 操作；公开输出和自动提交会跳过 `.env`、`.tokenpilot`、日志等 public-unsafe 路径

直驱模式面向本地 operator，不是面向公网的任意执行面。即使 `runShell` 通过白名单和参数校验限制了命令形态，它仍能运行 `node`、`python`、`npm` 等高能力工具；公网暴露时必须启用 bearer auth，并优先把复杂或长耗时任务交给 `createCodexRun`。
...
更完整的新手安装路径见 [`docs/deployment/beginner-quickstart.md`](./docs/deployment/beginner-quickstart.md)。

已有本地构建时，也可以使用原始 MVP 命令：
