...
Public network exposure belongs to a dedicated **Public Access / Connectivity** workbench in Web Cockpit. Web owns provider selection, domain/route intent, canonical Public Endpoint selection, reachability/TLS/DNS inspection, and staged cutover workflows. It does not install local binaries, mutate OS services, or render provider credentials in plaintext. The implemented workbench consumes a protected public-safe machine-provider projection, stages Candidate Public Routes separately from the canonical Runtime origin, explicitly verifies the exact current candidate through bounded public-unicast DNS plus pinned-address HTTPS checks, and can prepare/cancel a short-lived Cutover Intent bound to that exact successful Verification Artifact. Verification and Intent projections expose only bounded public-safe state; resolved IPs, raw TLS/network errors, response bodies, Runtime service execution, internal adapter identity, executable paths, raw provider output, mutation commands, and secrets remain outside Web. Actual cutover execution remains Machine Authority and is not implemented yet.
