...
| Runtime & Resource Center Inventory + governed mutation | Implemented | Schema v18 stores append-only public-safe inventory snapshots plus durable Resource mutation approvals, executions, and actor provenance. Native Codex, Downstream MCP, and ACP Registry adapters feed `/ui/resources`; governed Codex Skill enable/disable and Codex Plugin install/uninstall are available through operator REST/Resource Center and constrained MCP prepare/inspect/execute. Remote MCP cannot decide or reconcile, and exposed deployments require explicit Resource mutation exposure opt-in. |
