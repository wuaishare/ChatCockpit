...
- Implemented: SQLite Schema v15; Project, Workspace, Task, Development Session, generic Runtime Binding persistence for Codex Threads and TokenPilot Runner Job IDs, Runtime Recovery Attempt, append-only Runtime Resource Snapshot and Spec/Plan document versions, Task document foreign keys and immutable version pins, explicit `planning-required | planning-optional` Task execution policy, shared Spec/Plan, Runtime Recovery, and Runtime Resource application services, REST/MCP parity, Spec/Plan/Recovery Workbench governance, read-only Runtime & Resource Center, server-derived Planning and Recovery Assessment, Writer Lease, Handoff, Evidence, governed Task Review/Completion, Runtime Run, Runtime Approval, Direct Mutation Approval/Audit, Direct Command Approval/Audit, Direct Process Session/Approval/Audit, Process Supervisor Runtime Ownership, Event, Workspace Snapshot, and Continuity Workbench projections
...
- Target extensions: richer Task transitions; additional provider Recovery adapters including a future ACP seam; governed Resource Center install/update/enable/disable/authentication mutations; TDD/SDD/BDD orchestration and templates; and additional provider adapters
...
| Durable append-only Spec/Plan truth | Implemented | Schema v15 retains the fixed-kind documents, immutable Markdown versions, lifecycle status, Task foreign keys and immutable version pins introduced through Schema v7; shared REST/MCP services and the Workbench provide create/read/version/status/bind operations |
...
| Runtime Recovery Center | Implemented foundation | Schema v15 persists short-lived Recovery Attempts; Native Codex, Runner, and Chat Direct adapters provide explicit assess/execute flows with compatibility/hash drift protection and no implicit model turn |
| Runtime & Resource Center Inventory | Implemented read-only foundation | Schema v15 stores append-only public-safe inventory snapshots; Native Codex, Downstream MCP, and ACP Registry adapters feed `/ui/resources`, REST, and the two read-only MCP resource tools while install/update/enable/disable/authentication mutations remain gated |
