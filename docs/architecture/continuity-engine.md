...
- Current storage boundary: Runtime Resource inventory and mutation services depend on the narrow `GovernanceLedger` contract. The compatibility adapter still reuses the existing SQLite repositories, so persisted records and public behavior are unchanged while the Resource Center application layer no longer depends on the full Continuity repository aggregate.
...
| Durable append-only Spec/Plan truth | Implemented | Schema v18 retains the fixed-kind documents, immutable Markdown versions, lifecycle status, Task foreign keys and immutable version pins introduced through Schema v7; shared REST/MCP services and the Workbench provide create/read/version/status/bind operations |
...
| Runtime Recovery Center | Implemented foundation | Schema v18 persists short-lived Recovery Attempts; Native Codex, Runner, and Chat Direct adapters provide explicit assess/execute flows with compatibility/hash drift protection and no implicit model turn |
| Runtime & Resource Center Inventory + governed mutation | Implemented | Schema v18 stores append-only public-safe inventory snapshots plus durable Resource mutation approvals, executions, and actor provenance. Native Codex, Downstream MCP, and ACP Registry adapters feed `<console-path>/resources`; fresh initialization randomizes `<console-path>`. Governed Codex Skill enable/disable and Codex Plugin install/uninstall are available through operator REST/Resource Center and constrained MCP prepare/inspect/execute. Approve/deny requires an authenticated Operator session plus session-bound CSRF, including the receive-compatible `/tokenpilot/api/...` alias; Operator authority is independent of browser network locality; Machine API Bearer, MCP OAuth, and Remote MCP cannot decide. Exposed deployments require explicit Resource mutation exposure opt-in. |
