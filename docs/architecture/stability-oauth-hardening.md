...
7. existing MCP tools, including explicit Runtime Recovery assessment/execution, Direct Drive with governed Host Direct file read, approval-gated Write/Exact Edit, approval-gated bounded Host Command, Durable ChatCockpit-owned Managed Process lifecycle with Workspace Session/Writer-Lease governance plus OAuth Full Access-only Pure Host grant/actor authority, separate Process Supervisor restart continuity, offline Workspace Writer Lease enforcement and crash containment, Codex Session, Async Agent Job, Spec/Plan First, Completion, and Web flows do not regress; Recovery must continue to avoid implicit `turn/start` and automatic provider switching.
