...
**Platform Governance storage boundary — implemented:** `GovernanceLedger` is the logical dependency boundary for platform governance. Existing idempotency and Runtime Resource mutation/snapshot records remain backed by the current Continuity SQLite compatibility layer, while new provider-neutral governed external actions use a separate machine-local `governance.sqlite` owned by the Core governance layer. Governed external action approvals persist only target/provider/tool identity, argument hashes, public summaries, actor/request identity hashes, lifecycle timestamps/status, and execution outcome status; raw invocation arguments and provider result bodies are not stored.
