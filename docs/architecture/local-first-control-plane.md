...
- **Host Direct — implemented for governed Files and bounded Command:** Remote MCP can read small text-like files, perform approval-gated text Write / Exact Edit when the Root includes `write`, and run bounded non-interactive Host Commands through a separate Direct Command Approval lifecycle. File mutations keep canonical containment, symlink/sensitive-path checks, 64 KiB text limits, exact mutation-hash binding, and post-write verification. Host Command accepts structured `command + args + relative workdir` rather than an ungoverned raw-shell endpoint. Ordinary Host profiles keep the bounded Pure Host policy, while an explicit OAuth Full Access grant may run a general one-shot Host interpreter/command as an exact approved intent; Workspace write effects re-enter chat-direct Session, Writer Lease, Git, and Task Evidence governance. Public output is bounded and projected without PID/private cwd. System-wide arbitrary process attach/list/kill remains unexposed.
...
**Capability Router governed mutation — exposed through a stable governed surface:** provider-native `mutation` tools use a separate `prepare → Operator decide → execute` lifecycle backed by the Core Governance Ledger. Remote MCP exposes only the fixed ChatCockpit-owned `chatcockpit.capabilities.mutation.prepare`, `.inspect`, and `.execute` tools; there is no MCP `decide`, and provider-native tool names remain catalog data. Approve/deny is accepted only from an authenticated Operator session through `/api/capabilities/mutations/decision` (plus the receive-compatible `/tokenpilot` alias), with CSRF enforced on both paths; Operator authority is independent of browser network locality; anonymous, machine-bearer, MCP OAuth, and Remote MCP authority cannot decide. Approval records bind the local-device target, provider/tool identity, canonical argument hash, executor-config fingerprint, and inspected policy hash without persisting raw arguments. Before Approval consumption, execution opens the exact downstream client and performs live `tools/list` attestation on that same connection; schema/annotation drift marks the Approval stale with no provider call or Execution row, while transient start/attestation failure leaves it approved and retryable within TTL. Only after attestation succeeds does the prepared-external-mutation idempotency transaction consume Approval/create Execution and call the provider. Public output uses a dedicated bounded projection that excludes argument/policy/request hashes, arbitrary summaries, transport configuration, and raw provider errors/results.

**Durable Host Managed Process — implemented:** ChatCockpit keeps bounded managed processes behind public `host_process_*` identities while a separate local Process Supervisor sidecar owns the Desktop Commander stdio/PID namespace. Workspace-scope start/input use the owning chat-direct Session + Writer Lease and continue to record Workspace Process Audit/Task Evidence. Pure Host scope is separately available only to an explicit OAuth Full Access grant, is bound to the exact grant + actor through a Host Process Authority, requires the durable Supervisor, and has an independent global concurrency cap without inventing Workspace Session/Lease/Evidence. Both scopes expose only bounded public-safe read/list state/output, and stop remains available to the owning authority for cleanup. A normal Control Plane restart reconnects only when the same sidecar generation still owns the exact ChatCockpit process plus its scope-specific governance identity: Workspace/Task/Session/Writer Lease for Workspace scope, or Host Process Authority for Pure Host scope. The sidecar independently reads that scope-specific authority through read-only SQLite, journals terminal events for later Audit/Evidence ingestion, and wraps downstream MCP processes in a process-group guardian so a hard-killed sidecar cannot leave the managed child producing delayed side effects. Schema v13 permits sidecar-owned running records with `private_pid = NULL`; persisted PID is never a recovery credential. A new Supervisor generation never reattaches old runtimes by PID, and system-wide arbitrary process attach/list/kill remains intentionally unexposed. ChatCockpit Built-in and verified App Server Standalone providers are projected through normalized capability descriptors, health/scope metadata, public-safe discovery, and `automatic | explicit` selection. ChatCockpit remains the only remote MCP boundary exposed to ChatGPT. The Downstream MCP layer is also implemented: local-only executor config drives a stdio probe through the official `@modelcontextprotocol/client` transport/client lifecycle, explicit tool-to-capability mapping, a local capability snapshot, Broker descriptor projection, and normalized execution. ChatCockpit keeps policy, bounded stderr/buffer controls, public-safe error normalization, snapshotting, and capability mapping while the SDK owns stdio spawning, JSON-RPC framing, handshake, request correlation, schema validation, and transport teardown. Fresh downstream probes also persist a bounded local Tool Catalog with input/output schemas and annotations; legacy snapshots remain readable as summary-only metadata, and the catalog does not by itself expand the public Remote MCP tool surface. Downstream transports currently support local stdio and Streamable HTTP through the official MCP client SDK. Plain HTTP is restricted to loopback endpoints; non-loopback endpoints require HTTPS, and URL-embedded credentials/fragments are rejected. Desktop Commander managed-process lifecycle remains stdio-only because it depends on local process ownership rather than generic MCP transport semantics. Through the Desktop Commander adapter contract, governed Host execution covers `files.read`, `files.write`, `files.edit`, and bounded `shell.exec` mapped to current `start_process`. `read_process_output`, `interact_with_process`, and `force_terminate` remain private lifecycle dependencies. Raw downstream tool names, ungoverned raw-shell endpoints, raw downstream process control, system-wide arbitrary PID operations, and arbitrary downstream execution remain unexposed. Full Access may deliberately pass interpreter source as an exact structured command argument, but it does not reveal or delegate the downstream process namespace. All public Host execution stays behind ChatCockpit-owned scope, access-level checks, exact intent/authority binding, Approval where applicable, Workspace re-entry when applicable, Evidence/Audit, idempotency, timeout, output, and secret-safety rules.

**Codex App Server Standalone terminal truth — implemented:** `workspace.exec` no longer treats the original long-lived `command/exec` RPC response as the only terminal signal for a managed project process. ChatCockpit wraps the foreground command in the same verified Codex sandbox with a per-process random terminal token that is removed from the child environment before execution. When the foreground command exits, the wrapper emits a private terminal sentinel through the existing streamed output channel; ChatCockpit strips the sentinel from public output and uses it only to start a second provider cleanup fence. Every managed process is bound to the exact `CodexAppServerClient` generation that started it, so later write/resize/terminate/fence operations can never reconnect to a fresh App Server and mistake its empty process table for proof about the old process. Writer authority is released from the recovery path only after that same provider generation reports the process id as absent/not running. A successful `command/exec/terminate` acknowledgement by itself is not terminal truth: it only starts the same absence fence, and ChatCockpit remains `running` if the provider keeps acknowledging terminate while still owning the process. The original `command/exec` request is explicitly abandoned through an AbortSignal only after terminal proof, so a missing final RPC response can recover without a ghost writer authority while the same provider generation is still available to prove process absence. If the provider disconnects or the cleanup fence cannot be confirmed, ChatCockpit stays fail-closed: transport loss is recorded but does not become process-terminal truth, and writer authority is not released merely because the control channel disappeared. This preserves long-running foreground commands without introducing a blind process timeout; provider-loss recovery remains a separate containment problem rather than an implicit unlock.
...
- matching top-level user-facing Thread (provider `source=cli|vscode`, or an explicit `threadSource=user` analytics hint) → `chatcockpit.codex.thread.resume` is available as a session operation;
- no matching user-facing Thread, including when only exec/app-server/unknown or subagent Threads exist → `chatcockpit.codex.thread.start` is available as a session operation;
...
`chatcockpit.codex.thread.turn.start` is deliberately separate from those session operations. It may run only after an explicit Delegate/Transfer decision changes model-loop ownership to Codex. The Codex Thread ID remains authoritative for same-provider Codex continuity, but a ChatGPT takeover does not pretend to become native Codex history. New ChatCockpit-created user Threads send `threadSource=user` only as a provider analytics hint and apply best-effort native naming; automatic continuation primarily trusts the provider-native `SessionSource` plus top-level/subagent relationship rather than assuming that optional hint is persisted. Generic App Server still does not own Codex Desktop Project grouping. Provider writer ownership is respected and never stolen.

Native context reuse stays provider-first rather than duplicating Codex harness logic. `chatcockpit.codex.context.read` projects only public-safe facts from stable `config/read` and `skills/list`: configuration layer types, whether native instruction fields are configured, and bounded Skill metadata. ChatCockpit does not parse or copy provider instruction text. Explicit native Thread start/resume/fork responses may additionally expose a redacted summary of the instruction sources and runtime workspace roots already resolved by Codex, without returning external absolute paths.

Skill consumption preserves the same boundary. Codex remains authoritative for Skill discovery. When a discovered Skill file is physically contained by the registered Workspace, ChatCockpit may expose only its Workspace-relative path so the caller can reuse the existing governed `files.read` capability; external user/system Skill paths and contents remain private and are used only by Codex itself during an explicitly delegated native Turn. No separate Skill parser, Host-file bypass, or implicit model-loop transfer is introduced.

Project-scoped MCP applicability follows the same provider-native rule without depending on Thread lifecycle. `developmentCoordination.mcpApplicability` never treats the global Runtime Resource inventory as proof that an MCP server applies to a Project. For a ready Workspace, ChatCockpit calls stable `config/read` with the private Workspace cwd and projects only Codex's already-merged effective `mcp_servers` names plus enabled/disabled state. This yields configured, applicable/enabled, and disabled counts without parsing TOML, copying transport details, or creating/resuming a probe Thread. `mcpServerStatus/list` remains the authority for loaded-runtime tool/auth/health state in Resource Center or an active native runtime; that runtime status is intentionally not conflated with Project applicability. If the Workspace is not ready or the effective config cannot be observed within budget, counts remain `null` with an explicit not-required/degraded reason rather than being guessed from global inventory.

**Project Cockpit P0 — implemented:** the Web Cockpit Projects section reads `GET /api/continuity/projects/:projectId` as the authority for `developmentCoordination`. Its first screen is a read-only three-card projection of Project/Workspace/Git, model-loop/Codex/Handoff, and effective MCP applicability. It is intentionally independent of the Workspace Continuity Snapshot: snapshot availability must not hide project development coordination. The UI may translate server enums into product labels, but it does not infer model-loop ownership, Codex transfer policy, or MCP applicability. It does not consume compatibility `nativeDevelopment`, expose Thread preview text, or duplicate workflow mutations.
...
- Host Direct is exposed through public-safe Host Root Aliases for governed Files and bounded Host Command. Ordinary Host profiles remain conservative; explicit OAuth Full Access adds exact one-shot Host interpreters/commands plus ChatCockpit-owned Pure Host managed processes bound to that grant/actor and the durable Supervisor. Workspace write effects still require Workspace governance. There is no ungoverned raw-shell endpoint, arbitrary PTY/PID attachment, system-wide process listing/killing, or raw downstream Process Management surface.
