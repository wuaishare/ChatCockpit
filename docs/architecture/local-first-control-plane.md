...
**Platform Governance storage boundary — implemented:** `GovernanceLedger` is now the logical dependency boundary for platform governance while physical persistence deliberately remains in the existing machine-local Continuity SQLite during the reset migration. Provider-neutral governed external action tables are added through the same SQLite migration chain, so the architecture separates dependency direction without prematurely splitting storage. Governed external action approvals persist only target/provider/tool identity, argument hashes, public summaries, actor/request identity hashes, lifecycle timestamps/status, and execution outcome status; raw invocation arguments and provider result bodies are not stored.
