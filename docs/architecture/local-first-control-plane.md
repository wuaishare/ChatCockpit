...
**Capability Router read invocation — implemented internally:** an explicitly exposed `read` tool can be invoked only when the current catalog entry is ready, its bounded input schema is available, the supplied arguments pass the official MCP SDK JSON Schema validator, and downstream safety annotations do not conflict with read-only classification. Provider results are projected into bounded text/structured output; non-text content is omitted rather than proxied implicitly, and raw provider failures remain behind stable ChatCockpit error codes. This invocation seam is not yet published as a Remote MCP tool.

**Capability Router governed mutation — implemented internally:** provider-native `mutation` tools now use a separate `prepare → operator decide → execute` lifecycle backed by the Core Governance Ledger. Remote MCP may prepare an action and may execute it only after a non-Remote-MCP operator decision; it cannot approve its own action. Approval records bind the local-device target, provider/tool identity, canonical argument hash, executor-config fingerprint, and inspected policy hash without persisting raw arguments. Execution revalidates the current catalog, bounded schema, safety annotations, exposure mode, argument hash, executor-config fingerprint, and policy hash before consuming the approval. Completed execution is idempotently replayable only with the same key/input; ambiguous provider failures remain pending for explicit recovery instead of automatic replay. Provider failures are recorded as bounded execution status/error codes, and raw provider result bodies are never stored in governance tables. This mutation seam is still internal and is not yet exposed through Remote MCP or REST.
