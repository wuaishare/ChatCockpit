...
- 当前存储边界：Runtime Resource Inventory 与 Mutation Service 已只依赖窄化的 `GovernanceLedger` 合同。兼容适配层仍复用现有 SQLite Repository，因此持久化数据与公开行为不变，同时 Resource Center 应用层不再依赖完整 Continuity Repository 聚合。
