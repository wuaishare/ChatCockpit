...
- **Host Direct — 已实现受治理 Files 与 bounded Command：** Remote MCP 可以读取小型 text-like 文件，并在 Root 包含 `write` 时执行受审批的文本 Write / Exact Edit；同时可以通过独立 Direct Command Approval 生命周期执行 bounded non-interactive Host Command。文件变更继续受 canonical containment、symlink/sensitive-path 阻断、64 KiB 文本上限、exact mutation hash 与写后校验约束。Host Command 只接受结构化 `command + args + relative workdir`，不存在不受治理的 raw-shell 端点；普通 Host 档继续使用有界 Pure Host policy，而由 Owner 显式授予的 OAuth Full Access 可以把通用一次性 Host 解释器/命令作为精确 intent 执行；Workspace write effect 仍必须回流 chat-direct Session、Writer Lease、Git 与 Task Evidence。公共输出有界且不暴露 PID/private cwd；系统级任意进程 attach/list/kill 仍未开放。
...
**Resource Center Provider Management Read Model — 已实现：** 现有 Runtime Profile Endpoint 现在会同时返回 public-safe 的 `local-device` Target、标准化 Provider Descriptor、原有 `profiles` 兼容字段，以及新增的 `management` 投影。Management Projection 统一表达检测、版本、健康、Capabilities、配置来源、Chat Exposure、Desired/Observed State、允许的 Lifecycle Operations 与 Provider-native Verification，同时不暴露下游 Command、Args、Env、Transport Endpoint、Credential 或 Raw Provider Config。已配置但没有有效 Probe 的 Downstream MCP Provider 明确保持 `unverified`；Protocol 变化会让旧 Observation 变为 `stale`，不会继续伪装成当前真相。Web Resource Center 现在按 Provider Management → Owner-only 实时 Operational Activity → Runtime Profiles 的顺序组织；Activity 驾驶舱只在投影能够证明当前 Authority 与 Revision 时开放经审查的控制：存在 Active Bound Codex Turn 时，Owner 可通过既有、带 Revision 与 Idempotency 约束的 Codex Interrupt 合同显式中断当前运行；存在 Tracked Job Process 时，则通过正式的 `POST /api/jobs/{id}/control` 合同按当前 Process Revision 与 Idempotency Key 暂停、继续或终止任务。成功的 Job Control 会追加一条 Governance Control Event，只记录 Job ID、Action、结果 State/Revision、经过 Hash 的 Actor/Request Provenance 与时间戳；这些事件再标准化成 public-safe `activity.event` 推送给实时驾驶舱，不记录或推送 PID、Command、Path、Job Instructions 或 Output。旧 `/{action}` 路由仅作为 Deprecated Compatibility 保留，内部同样经过这一服务。现有 Resource Inventory 与 Mutation 流程仍以 Runtime Profile 为 Authority。
...
**Capability Router Governed Mutation — 已通过稳定受治理 Surface 暴露：** Provider-native `mutation` Tool 使用独立的 `prepare → Operator decide → execute` 生命周期，并由 Core Governance Ledger 承载。Remote MCP 只固定注册 ChatCockpit-owned 的 `chatcockpit.capabilities.mutation.prepare`、`.inspect`、`.execute`，不注册 MCP `decide`，Provider-native Tool Name 仍只是 Catalog 数据。Approve/Deny 只接受已认证 Operator Session 通过 `/api/capabilities/mutations/decision`（以及 receive-compatible `/tokenpilot` alias）提交，并且两条路径都强制 CSRF；Operator Authority 与浏览器网络位置无关，不等同于 localhost 限制；Anonymous、Machine Bearer、MCP OAuth、Remote MCP 都不能 Decide。Approval 绑定 local-device Target、Provider/Tool Identity、Canonical Arguments Hash、Executor Config Fingerprint 与已检查 Policy Hash，但不持久化原始参数。Consume Approval 之前会先创建将要执行写操作的精确 Downstream Client，并在同一连接上做 live `tools/list` attestation；Schema/Annotation 漂移会把 Approval 标记为 stale，同时保持 0 Provider Call / 0 Execution，临时 Start/Attestation Failure 则保持 approved，可在 TTL 内重试。只有 Attestation 通过后，prepared-external-mutation 幂等事务才会 Consume Approval / Create Execution 并调用 Provider。公共结果统一经过专用 bounded Projection，不返回 Arguments/Policy/Request Hash、任意 Summary、Transport Config 或原始 Provider Error/Result。

**Durable Host Managed Process — 已实现：** ChatCockpit 通过公共 `host_process_*` 身份管理受控托管进程，真实 Desktop Commander stdio/PID namespace 由独立本机 Process Supervisor sidecar 持有。Workspace scope 的 Start/Input 绑定 owning chat-direct Session + Writer Lease，并继续记录 Workspace Process Audit/Task Evidence；Pure Host scope 则只对 Owner 显式授予的 OAuth Full Access 开放，通过 Host Process Authority 精确绑定 authorization grant + actor，要求 durable Supervisor，并使用独立的全局并发上限，不伪造 Workspace Session/Lease/Evidence。两种 scope 的 Read/List 都只返回有界 public-safe 状态/输出，Stop 保留用于所属 authority 的安全清理。普通 Control Plane restart 只有在同一个 sidecar generation 仍拥有完全一致的 ChatCockpit Process 与该 scope 对应的治理身份时才允许重连：Workspace scope 校验 Workspace / Task / Session / Writer Lease，Pure Host scope 校验 Host Process Authority。Sidecar 会通过 read-only SQLite 独立检查这套 scope-specific authority，把离线期间的 terminal event 记入本地 journal，待 Control Plane 恢复后幂等回流 Audit/Evidence；Downstream MCP 外层还有 process-group guardian，因此真实 hard-kill sidecar 后 managed child 不能继续产生延迟副作用。Schema v13 允许 sidecar-owned `running` 记录保持 `private_pid = NULL`；persisted PID 从来不是恢复凭据，新 Supervisor generation 也不会根据旧 PID 重连。系统级任意进程 attach/list/kill 继续明确不开放。ChatCockpit Built-in 与经过验证的 App Server Standalone 会被标准化为统一 Capability Descriptor，并提供 Health / Scope、public-safe discovery 与 `automatic | explicit` Provider Selection。ChatGPT 对外仍只连接 ChatCockpit Remote MCP。Downstream MCP 层也已实现：local-only Executor 配置通过官方 `@modelcontextprotocol/client` Transport/Client 生命周期驱动 stdio Probe，以显式 Tool → Capability Mapping 生成本地 Capability Snapshot，再投影 `downstream-mcp` Descriptor 并执行 normalized capability。ChatCockpit 继续负责 Policy、stderr/buffer 上限、public-safe Error Normalization、Snapshot 与 Capability Mapping；stdio Spawn、JSON-RPC Framing、Handshake、Request Correlation、Schema Validation 与 Transport Teardown 交由官方 SDK。新的 Downstream Probe 还会在本机持久化有界的 Tool Catalog，包括 input/output schema 与 annotations；旧 Snapshot 仍按 summary-only 元数据兼容读取，而且这份 Catalog 本身不会扩大公开 Remote MCP Tool Surface。Downstream Transport 当前通过官方 MCP Client SDK 同时支持本机 stdio 与 Streamable HTTP；明文 HTTP 仅允许 loopback，非 loopback Endpoint 必须使用 HTTPS，同时拒绝 URL 内嵌凭据与 fragment。Desktop Commander Managed Process 生命周期继续保持 stdio-only，因为它依赖本机进程 Ownership，而不是通用 MCP Transport 语义。当前 Desktop Commander Adapter Contract 已受治理地覆盖 Host `files.read`、`files.write`、`files.edit`，以及映射到当前 `start_process` 的 bounded `shell.exec`；`read_process_output`、`interact_with_process` 与 `force_terminate` 只作为 Adapter 私有生命周期依赖。Raw downstream tool name、不受治理的 raw-shell 端点、raw downstream 进程控制、系统级任意 PID 操作和任意下游执行都不暴露。Full Access 可以有意把解释器源码作为精确结构化命令参数传入，但不会暴露或下放 downstream 进程命名空间；公共 Host 执行仍统一受 Scope、Access Level、精确 intent/authority 绑定、适用时的 Approval/Workspace re-entry、Evidence/Audit、Idempotency、Timeout、Output Bound 与 Secret Safety 规则约束。
...
- Host Direct 已通过 public-safe Host Root Alias 开放受治理 Files 与 bounded Host Command。普通 Host 档保持保守；Owner 显式授予的 OAuth Full Access 额外开放精确的一次性 Host 解释器/命令，以及绑定该 grant/actor 与 durable Supervisor 的 ChatCockpit-owned Pure Host 托管进程。Workspace write effect 仍走 Workspace 治理；不提供不受治理的 raw-shell 端点、任意 PTY/PID attach、系统级进程 list/kill 或 raw downstream Process Management；
