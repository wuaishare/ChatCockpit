...
**Capability Router Governed Mutation — 已通过稳定受治理 Surface 暴露：** Provider-native `mutation` Tool 使用独立的 `prepare → local operator decide → execute` 生命周期，并由 Core Governance Ledger 承载。Remote MCP 只固定注册 ChatCockpit-owned 的 `chatcockpit.capabilities.mutation.prepare`、`.inspect`、`.execute`，不注册 MCP `decide`，Provider-native Tool Name 仍只是 Catalog 数据。Approve/Deny 只接受已认证本地 Operator Session 通过 `/api/capabilities/mutations/decision`（以及 receive-compatible `/tokenpilot` alias）提交，并且两条路径都强制 CSRF；Anonymous、Machine Bearer、MCP OAuth、Remote MCP 都不能 Decide。Approval 绑定 local-device Target、Provider/Tool Identity、Canonical Arguments Hash、Executor Config Fingerprint 与已检查 Policy Hash，但不持久化原始参数。Consume Approval 之前会先创建将要执行写操作的精确 Downstream Client，并在同一连接上做 live `tools/list` attestation；Schema/Annotation 漂移会把 Approval 标记为 stale，同时保持 0 Provider Call / 0 Execution，临时 Start/Attestation Failure 则保持 approved，可在 TTL 内重试。只有 Attestation 通过后，prepared-external-mutation 幂等事务才会 Consume Approval / Create Execution 并调用 Provider。公共结果统一经过专用 bounded Projection，不返回 Arguments/Policy/Request Hash、任意 Summary、Transport Config 或原始 Provider Error/Result。
