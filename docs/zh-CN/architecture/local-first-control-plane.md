...
**Host Managed Workspace Process — 已实现：** TokenPilot 可以通过公共 `host_process_*` 身份持续管理一个受控的 Workspace 交互进程。Start 与 Input 必须由拥有 chat-direct Session + Writer Lease 的 Workspace 发起；原 owner 丢失 Lease 后仍可执行 Stop 做安全清理；Read/List 只返回有界的 public-safe 状态与输出。Desktop Commander PID 和其内部 Process Session Table 始终是私有 Adapter State；Control Plane 重启后无法可靠恢复的历史 Runtime 只会标记 stale，不会根据持久 PID 猜测重连；系统级任意进程 attach/list/kill 继续明确不开放。TokenPilot Built-in 与经过验证的 App Server Standalone 会被标准化为统一 Capability Descriptor，并提供 Health / Scope、public-safe discovery 与 `automatic | explicit` Provider Selection。ChatGPT 对外仍只连接 TokenPilot Remote MCP。Downstream MCP 层也已实现：local-only Executor 配置驱动 stdio Probe，通过官方 MCP Schema 校验后，以显式 Tool → Capability Mapping 生成本地 Capability Snapshot，再投影 `downstream-mcp` Descriptor 并执行 normalized capability。当前 Desktop Commander Adapter Contract 已受治理地覆盖 Host `files.read`、`files.write`、`files.edit`，以及映射到当前 `start_process` 的 bounded `shell.exec`；`read_process_output` 与 `force_terminate` 只作为 Adapter 私有生命周期依赖。Raw downstream tool name、raw shell source、raw downstream 进程控制、系统级任意 PID 操作和任意下游执行都不暴露；公共 Host 执行仍统一受 Scope、Approval、Workspace re-entry、Writer Lease、Git、Evidence/Audit、Idempotency、Timeout、Output Bound 与 Secret Safety 规则约束。
