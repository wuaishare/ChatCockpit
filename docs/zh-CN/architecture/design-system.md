...
业务组件只允许表达五种实现层 Tone：`default`、`processing`、`success`、`warning`、`error`。`blue`、`green`、`orange`、`gold`、`red`、`purple`、`cyan` 等 Ant Design 命名 preset 色属于底层色板实现细节，不得直接进入产品业务组件逻辑。
...
浅色主题使用低饱和冷白和蓝灰，而不是营销页面式纯白。普通 Panel 采用“边框优先、默认无投影”的结构，只有真正需要抬升层级的 Hero / Overlay 才使用克制阴影，不让每一张卡片都形成悬浮层。

重复的 Section Header 只有在图标确实传递信息时才使用强调色图标。把同一个彩色装饰图标复制到每个区块不属于层级设计；默认层级应由字体、间距和内容结构承担。
