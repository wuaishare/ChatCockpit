...
- Implemented capabilities: Continuity Engine through Schema v11, Writer Lease, Handoff/Evidence, governed Task Review/Completion, explicit Codex Turn/Approval, Direct Mutation Approval/Audit, Direct Command Approval/Audit, Workspace Snapshot, Continuity-bound Async Job Queue, Runner lifecycle/restart reconciliation, versioned Spec/Plan truth with immutable Task version pins, explicit planning-required/planning-optional execution policy, server-derived Planning Assessment, 58 MCP tools including Direct Drive executor discovery, Host Root Alias discovery, Host Direct file read, approval-gated Write/Exact Edit, approval-gated bounded Host Command lifecycle, and TokenPilot-owned Managed Workspace Process lifecycle, Spec/Plan/Completion/Runtime Workbench UX, Queue/Runner, and public-safe artifacts
