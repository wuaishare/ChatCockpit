...
- Implemented capabilities: Continuity Engine through Schema v15, Writer Lease, Handoff/Evidence, governed Task Review/Completion, explicit Codex Turn/Approval, Runtime Recovery Assessment/Attempt/Execution, Native Codex Recovery with compatibility gating, Runner and Chat Direct recovery projections, append-only Runtime Resource Inventory Snapshot truth, Native Codex Skills/MCP/Plugins/config inventory, Downstream MCP resource inventory, ACP Registry Agent catalog, Direct Mutation Approval/Audit, Direct Command Approval/Audit, Workspace Snapshot, Continuity-bound Async Job Queue, Runner lifecycle/restart reconciliation, versioned Spec/Plan truth with immutable Task version pins, explicit planning-required/planning-optional execution policy, server-derived Planning Assessment, 62 MCP tools including `tokenpilot.recovery.assess` / `tokenpilot.recovery.execute` and read-only `tokenpilot.resources.inventory` / `tokenpilot.resources.inspect`, Direct Drive executor discovery, Host Root Alias discovery, Host Direct file read, approval-gated Write/Exact Edit, approval-gated bounded Host Command lifecycle, and Durable TokenPilot-owned Managed Workspace Process lifecycle with separate Process Supervisor generation/ownership, offline Writer Lease watchdog, terminal-event reconciliation, and downstream process-group crash containment, plus Spec/Plan/Completion/Runtime Recovery Workbench UX, `/ui/resources` Resource Center, Queue/Runner, and public-safe artifacts
...
- Known limitations: no native installer, no public SaaS, Resource Center mutations (install/update/enable/disable/authentication) remain Phase 6B gated, no full TDD/SDD/BDD orchestration layer, no generic transition service for every Task edge, and no generic ACP recovery adapter yet; Recovery remains explicit, never auto-starts `turn/start`, and never silently switches providers
...
- `/ui/resources` renders real Runtime Profiles and read-only inventory state; the public catalog remains 62 tools and Resource Center mutation actions are absent.
