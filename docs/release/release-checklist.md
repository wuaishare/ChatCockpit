...
- Implemented capabilities: Continuity Engine through Schema v8, Writer Lease, Handoff/Evidence, governed Task Review/Completion, explicit Codex Turn/Approval, Direct Mutation Approval/Audit, Workspace Snapshot, Continuity-bound Async Job Queue, Runner lifecycle/restart reconciliation, versioned Spec/Plan truth with immutable Task version pins, explicit planning-required/planning-optional execution policy, server-derived Planning Assessment, 50 MCP tools including Direct Drive executor discovery, Host Root Alias discovery, Host Direct file read and approval-gated Write/Exact Edit lifecycle, Spec/Plan/Completion/Runtime Workbench UX, Queue/Runner, and public-safe artifacts
