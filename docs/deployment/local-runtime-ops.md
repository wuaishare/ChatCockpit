...
- The authenticated Dashboard and Public Access view also project **LAN Cockpit** readiness. A LAN URL is emitted only when Trusted LAN is enabled, the Runtime listener actually admits the relevant address family/address, and a non-loopback interface address falls inside the configured trusted CIDRs. The Web UI never widens the listener or enables Trusted LAN implicitly.
