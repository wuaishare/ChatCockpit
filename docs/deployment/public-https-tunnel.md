...
Custom GPT Actions cannot reach `127.0.0.1` on your computer. To let ChatGPT call local ChatCockpit, you need a public HTTPS URL that forwards to the local control plane.
...
- ChatCockpit runs with `CHATCOCKPIT_EXPOSED=true`.
- `CHATCOCKPIT_API_TOKEN` is set and matches GPT Builder Authentication.
- `CHATCOCKPIT_PUBLIC_BASE_URL` matches the OpenAPI server URL imported by GPT Builder.
...
ChatCockpit does not require a specific provider.
...
The public repository should only use placeholders such as `https://chatcockpit.example.com`.
...
| 401 | Token mismatch | Check GPT Builder Authentication and `CHATCOCKPIT_API_TOKEN` |
...
- Do not expose ChatCockpit as an unauthenticated public service.
