...
export const directProcessStartingMigration = {
  version: 11,
  name: "direct-host-process-start-reservations",
  foreignKeysOff: true,
  up(database: DatabaseSync): void {
    database.exec(`
      CREATE TABLE direct_process_sessions_v11 (
        id TEXT PRIMARY KEY,
        ...
        CHECK (
          status IN ('starting', 'failed', 'stale') OR private_pid IS NOT NULL
        )
      ) STRICT;

      INSERT INTO direct_process_sessions_v11 (
      ...
      ALTER TABLE direct_process_sessions_v11 RENAME TO direct_process_sessions;
