...
export function buildGptInstructions(
  ...
  if (locale === "en-US") {
    return projectGptIdentityText([
      ...
      "- nativeDevelopment=start-native → start a new user-facing provider-native Codex Thread (supply a concise task/project name when useful), then start its native Turn.",
      "- nativeDevelopment=resume-native → resume the same authoritative top-level user Codex Thread returned by routing, then start its native Turn; do not substitute legacy/non-user or subagent Threads.",
  ...
  return projectGptIdentityText([
    ...
    "- nextAction=start-native → 创建新的顶层 user-facing Provider-Native Codex Thread（合适时传入简短任务/项目名称），再启动原生 Turn。",
    "- nextAction=resume-native → 只 Resume 路由返回的同一个顶层 user Codex Thread，再启动原生 Turn；不要用 legacy/non-user 或 subagent Thread 替代。",
