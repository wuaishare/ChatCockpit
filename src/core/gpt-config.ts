...
export function buildGptInstructions(
  ...
  if (locale === "en-US") {
    return projectGptIdentityText([
      ...
      "- Never start a provider-native Codex Turn unless the operator explicitly delegates/transfers model-loop ownership to Codex. On the canonical Core, use chatcockpit.codex.invoke and set the required modelLoopTransfer assertion only after that explicit operator decision.",
  ...
  return projectGptIdentityText([
    ...
    "- 只有用户明确要求 Delegate/Transfer 到 Codex 时，才允许启动 provider-native Codex Turn；canonical Core 使用 chatcockpit.codex.invoke，并且只有在该显式用户决定之后才能填写必需的 modelLoopTransfer 声明。模型循环所有权变化时必须做显式 Handoff/Transfer。",
...
export function buildGptConfig(
  ...
  return {
    ...
    notes: [
      ...
      "Codex Thread Start/Resume 是 provider-native continuity fast path；canonical Core 通过有界 chatcockpit.codex.invoke 调用这些原生动作，只有用户明确 Delegate/Transfer 到 Codex 时才携带 modelLoopTransfer 并启动 native Turn。",
