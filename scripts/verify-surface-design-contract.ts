...
assertIncludesAll(
  ...
  [
    ...
    "Connections",
    "This Host / This Mac",
    "Work is an IA grouping, not a merged persistence model",
    "Route stability does not make a route family a canonical top-level Product concept",
...
assertIncludesAll(
  ...
  [
    ...
    "Connections / 连接",
    "This Host / 本机",
    "Work 只是 IA 分组，不合并持久化模型",
    "路由稳定不代表该 route family 就是 canonical 一级产品概念",
    "## Capability Placement Matrix"
  ],
...
assert.match(projectCenter, /selectedProjectTargetExecutable/);
assert.match(projectCenter, /<ProjectActionTargetList/);
assert.match(projectCenter, /onSelectLocalTarget=\{\(target\) => setAddTargetId\(target\.deviceId\)\}/);
assert.doesNotMatch(projectCenter, /localProjectAvailable/);
