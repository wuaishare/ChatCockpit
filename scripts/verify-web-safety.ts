...
assert.match(appSource, /<nav className="app-toolbar__group app-toolbar__group--views"/);
assert.match(appSource, /copy\.header\.workspaceNavigation/);
assert.match(appSource, /copy\.header\.systemNavigation/);
assert.match(appSource, /app-nav-cluster--system/);
assert.match(appSource, /activeView === "dashboard"[\s\S]*activeView === "jobs"[\s\S]*: undefined/s);
assert.match(appSource, /activeView === "publicAccess" \|\| activeView === "integrations"[\s\S]*: undefined/s);
assert.match(stylesSource, /\.app-toolbar__group--views\s*\{[^}]*overflow-x:\s*auto/s);
assert.match(stylesSource, /\.app-nav-cluster--system\s*\{[^}]*border-left:\s*1px solid var\(--tp-line\)/s);
assert.match(stylesSource, /@media \(max-width: 640px\)[\s\S]*\.app-nav-cluster__label\s*\{[^}]*display:\s*none/s);
