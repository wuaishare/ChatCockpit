...
assert.match(appSource, /allowedKeys\s*=\s*new Set\(\["request_id", "ui_locales"\]\)/);
assert.match(appSource, /some\(\(key\) => !allowedKeys\.has\(key\)\)/);
assert.match(appSource, /uiLocales === "zh-CN" \|\| uiLocales === "en-US"/);
