...
assert.match(appSource, /readOAuthLoginBootstrap/);
assert.match(appSource, /params\.get\("oauth_request_id"\)/);
assert.match(appSource, /locale === "zh-CN" \|\| locale === "en-US"/);
assert.match(appSource, /\^oauth_request_/);
assert.match(appSource, /window\.location\.assign\(`\/oauth\/authorize\?\$\{params\.toString\(\)\}`\)/);
assert.doesNotMatch(appSource, /returnTo/);
assert.match(apiSource, /X-ChatCockpit-OAuth-Request-Id/);
...
assert.match(consolePathSource, /CONSOLE_BASE_PATH\s*=\s*DEFAULT_CONSOLE_BASE_PATH/);
assert.doesNotMatch(consolePathSource, /querySelector/);
...
assert.match(sidebarSource, /type: "group"[\s\S]*labels\.workNavigation/s);
assert.match(sidebarSource, /type: "group"[\s\S]*labels\.executionNavigation/s);
assert.match(sidebarSource, /type: "group"[\s\S]*labels\.connectionsNavigation/s);
assert.match(sidebarSource, /key: "overview"[\s\S]*label: labels\.dashboard/);
assert.match(sidebarSource, /key: "runtime"[\s\S]*label: labels\.runtime/);
assert.match(sidebarSource, /key: "connectionsPublicAccess"[\s\S]*label: labels\.publicAccess/);
assert.match(appSource, /aria-label=\{copy\.header\.openNavigation\}/);
assert.match(sidebarSource, /inlineCollapsed=\{compact\}/);
assert.match(sidebarSource, /<Drawer/);
assert.match(sidebarSource, /selectedKeys=\{\[activeNavigationKey\]\}/);
