...
assert.match(api, /createDevicePairing/);
assert.match(api, /"\/api\/devices\/pairings"/);
...
assert.match(types, /locality:\s*"local" \| "remote"/);
...
assert.match(view, /createDevicePairing\(name\)/);
assert.match(view, /revokeDevice\(deviceId\)/);
assert.match(view, /window\.setInterval\([^]*30_000/);
...
assert.match(view, /pairing\.code/);
assert.match(view, /CopyButton/);
...
assert.match(copy, /不通过局域网扫描猜测设备/);
assert.match(copy, /signed device heartbeats rather than LAN scanning/);
assert.match(copy, /远程启停尚未开放/);
assert.match(copy, /Remote start\/stop not enabled yet/);

assert.match(routes, /app\.get\("\/api\/devices"/);
assert.match(routes, /app\.post\("\/api\/devices\/pairings"/);
assert.match(routes, /app\.post\("\/api\/devices\/pairings\/claim"/);
