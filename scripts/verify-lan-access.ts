...
assert.deepEqual(ready.cockpitUrls, ["http://198.51.100.10:4318/cc-lan-fixture"]);

const explicitHost = buildLanAccessSnapshot({
  policy: policy(true, ["198.51.100.0/24"]),
  host: "198.51.100.20",
  port: 5123,
  consolePathPrefix: "/ops",
  addresses: ["198.51.100.10", "198.51.100.20"]
});
assert.equal(explicitHost.status, "ready");
assert.deepEqual(explicitHost.cockpitUrls, ["http://198.51.100.20:5123/ops"]);
...
assert.deepEqual(ipv6.cockpitUrls, ["http://[2001:db8:1234::10]:4318/cc-lan-fixture"]);
