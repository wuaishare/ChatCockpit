...
assert.match(integrations, /工具快照诊断/);
assert.match(integrations, /ChatCockpit 无法读取 ChatGPT 当前会话缓存的工具快照/);
assert.match(integrations, /Tool snapshot diagnostics/);
assert.match(integrationsView, /status\.mcp\.coreToolCount/);
assert.match(integrationsView, /status\.mcp\.fullToolCount/);
assert.match(integrationsView, /status\.mcp\.serverVersion/);
assert.match(integrationsView, /status\.mcp\.toolCatalogFingerprint/);
assert.match(integrationsView, /status\.mcp\.toolsInvokeAvailable/);
...
assert.doesNotMatch(api, /"X-ChatCockpit-Console-Path"/);
assert.match(api, /"X-ChatCockpit-Login-Gate"/);
assert.match(app, /readSecureLoginGate/);
