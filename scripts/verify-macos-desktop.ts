...
assert.doesNotMatch(statusView, /SettingsView\(model: model, scope: \.runtime\)/);
assert.doesNotMatch(statusView, /SettingsView\(model: model, scope: \.projects\)/);
assert.doesNotMatch(statusView, /NativeSharedCockpitBridgeView\(model: model, destination: \.work\)/);
assert.doesNotMatch(statusView, /NativeSharedCockpitBridgeView\(model: model, destination: \.resources\)/);
assert.doesNotMatch(statusView, /NativeSharedCockpitBridgeView\(model: model, destination: \.devices\)/);
assert.doesNotMatch(statusView, /NativeConnectionsBridgeView\([\s\S]*mainSection: \$selection[\s\S]*operationalSettingsFocus: \$operationalSettingsFocus[\s\S]*\)/s);
...
assert.doesNotMatch(statusView, /overviewColumns|OverviewCard|ServiceHealthTile|OperationalMetricTile/);
assert.doesNotMatch(statusView, /overallState\.nativeSemantic|state\.nativeSemantic|overviewSemantic/);
assert.doesNotMatch(statusView, /AccessibleTextActionButton/);
assert.doesNotMatch(statusView, /snapshot\.executionWorkspaceAvailable[\s\S]*(?:Start Services|Restart Services)/s);
...
assert.doesNotMatch(statusView, /DesktopL10n\.string\("Local Cockpit"\)/);
assert.doesNotMatch(statusView, /DesktopL10n\.string\("Public Cockpit"\)/);
assert.doesNotMatch(statusView, /snapshot\.localCockpitURL|snapshot\.publicCockpitURL/);
assert.doesNotMatch(statusView, /currentAppProvenanceText/);
...
assert.match(buildProvenanceStamp, /dist\/build-provenance\.json/);
assert.match(buildProvenanceStamp, /read_provenance_field buildId/);
assert.match(buildProvenanceStamp, /PROVENANCE_REVISION/);
assert.match(buildProvenanceStamp, /PROVENANCE_BUILT_AT/);
assert.match(buildProvenanceStamp, /%y%m%d%H%M%S/);
...
assert.doesNotMatch(statusView, /cockpitEndpointRow|AccessibleIconButton|activityCard|OperationalMetricTile/);
assert.doesNotMatch(statusView, /copyMachineEndpoint|ownerSummary|machineTokenSummary|securityFeedback/);
assert.doesNotMatch(statusView, /runtimeArchitectureText|runtimeVersionText|currentAppProvenanceText|updateStatusText/);
...
assert.match(settings, /case thisMac/);
assert.doesNotMatch(settings, /case runtime\b|case projects\b/);
assert.doesNotMatch(settings, /showsRuntime|showsProjects/);
...
assert.doesNotMatch(settings, /Text\(verbatim: model\.endpointText\)/);
...
assert.doesNotMatch(settings, /Import Existing Setup…|never migrated/);
assert.match(settings, /DesktopL10n\.string\("Security & Access"\)/);
assert.doesNotMatch(settings, /Section\(DesktopL10n\.string\("Projects"\)\)/);
assert.doesNotMatch(settings, /Section\(DesktopL10n\.string\("Project Roots"\)\)/);
assert.doesNotMatch(settings, /selectPackagedProject|selectedPackagedProjectRoots|addProjectRootFromPanel|makeProjectRootPrimary|removeProjectRoot|openProjectCenter/);
assert.doesNotMatch(settings, /DesktopL10n\.string\("Add Project…"\)|DesktopL10n\.string\("Add Root…"\)|DesktopL10n\.string\("Open Project Center"\)/);
...
assert.doesNotMatch(statusView, /DesktopCockpitSessionBuilder\(\)\.directURL\(/);
assert.doesNotMatch(statusView, /components\.path = destination\.targetPath/);
assert.doesNotMatch(embeddedRendererPolicy, /CHATCOCKPIT_DESKTOP_SHARED_RENDERER_M2/);
assert.doesNotMatch(embeddedRendererPolicy, /DesktopSharedRendererPrototypeConfiguration/);
...
assert.doesNotMatch(statusView, /NavigationSplitView/);
assert.doesNotMatch(statusView, /Shared Renderer Prototype/);
assert.doesNotMatch(statusView, /DesktopSharedRendererPrototypeConfiguration/);
...
assert.match(sharedCockpitWebView, /sameOrigin\(currentURL, baseURL\)/);
assert.match(sharedCockpitWebView, /sameOrigin\(nextURL, baseURL\)/);
assert.match(sharedCockpitWebView, /struct SharedCockpitView: View/);
assert.doesNotMatch(sharedCockpitWebView, /SharedCockpitPrototypeView/);
assert.match(sharedCockpitWebView, /destination: DesktopCockpitDestination\?/);
assert.match(sharedCockpitWebView, /DesktopCockpitSessionBuilder\(\)\.directURL\([\s\S]*destination: destination/);
assert.match(sharedCockpitWebView, /case loadFailed\(URL\)/);
assert.doesNotMatch(sharedCockpitWebView, /case loadFailed\(URL, URL\)/);
assert.match(sharedCockpitWebView, /guard context\.coordinator\.requestedURL != requestedURL else \{ return \}/);
assert.match(sharedCockpitWebView, /context\.coordinator\.navigate\(webView, to: requestedURL\)/);
assert.match(sharedCockpitWebView, /requestedURL = nextURL/);
assert.match(sharedCockpitWebView, /webView\.load\(URLRequest\(url: requestedURL\)\)/);
assert.match(sharedCockpitWebView, /let safeURL = DesktopCockpitSessionBuilder\(\)\.directURL\([\s\S]*destination: destination/);
assert.match(sharedCockpitWebView, /NSWorkspace\.shared\.open\(safeURL\)/);
assert.doesNotMatch(sharedCockpitWebView, /NSWorkspace\.shared\.open\(requestedURL\)/);
...
assert.doesNotMatch(settings, /documentation or knowledge root can be primary without pretending to be a checkout/);
...
assert.doesNotMatch(settings, /DesktopL10n\.string\("Open Local Cockpit"\)/);
assert.doesNotMatch(settings, /DesktopL10n\.string\("Open Public Cockpit"\)/);
