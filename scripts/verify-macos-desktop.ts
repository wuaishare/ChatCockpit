...
assert.match(appEntry, /private struct AccessibleTextActionButton: NSViewRepresentable/);
...
assert.match(statusView, /NSCursor\.pointingHand\.set\(\)/);
assert.match(statusView, /\.focusable\(true\)/);
assert.match(statusView, /\.accessibilityHint\(DesktopL10n\.string\("Open in Browser"\)\)/);
...
assert.match(settings, /TextField\("\/ui", text: \$consolePathPrefix\)/);
